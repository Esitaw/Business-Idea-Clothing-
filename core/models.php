...
function insertPurchase($pdo, $customer_name, $item_name, $price, $designer_id) {
    $sql = "INSERT INTO purchases (customer_name, item_name, price, designer_id) VALUES (?, ?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    $executeQuery = $stmt->execute([$customer_name, $item_name, $price, $designer_id]);
    if ($executeQuery) {
        return true;
    }
}
